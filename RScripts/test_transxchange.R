...
gtfs_batch <- pbapply::pblapply(res_batch[y], transxchange_export, run_debug = T, cal = cal, naptan = naptan)
...
write_gtfs(gtfs = gtfs_merged, folder = "../UK2GTFS-extras/export/", name = "EA_redo")
...
run_debug = T
full_import = F
naptan = get_naptan(naptan_extra = UK2GTFS::naptan_missing)
cal = get_bank_holidays()
for(i in 11:length(dir)){

  if(file.exists(paste0("../../mem48/UK2GTFS-extras/export/",dir[i],".zip"))){
    ...
    gtfs_merged <- gtfs_merge(gtfs_batch)
    write_gtfs(gtfs = gtfs_merged, folder = "../../mem48/UK2GTFS-extras/export/", name = dir[i])
  }

}
# Worked: EA, EM, S, SE, L, EM
# Fail, NE, NW, SW, W,

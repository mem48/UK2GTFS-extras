...
gtfs_merge <- function(gtfs_list){
  ...
  stops$file_id <- NULL
  stops <- unique(stops)
  if(any(duplicated(stops$stop_id))){stop("Duplicated Stop IDS")}

  #routes
  if(any(duplicated(routes$route_id))){
    message("De-duplicating route_id")
    route_id <- routes[,c("file_id","route_id")]
    if(any(duplicated(route_id))){stop("Duplicated route_id within the same GTFS file")}
  ...
  if(any(duplicated(calendar$service_id))){
    ...
    if(any(duplicated(service_id))){stop("Duplicated service_id within the same GTFS file")}
  ...
  if(any(duplicated(trips$trip_id))){
    ...
    if(any(duplicated(trip_id))){stop("Duplicated trip_id within the same GTFS file")}
